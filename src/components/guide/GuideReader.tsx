  ...
  return (
    <div className="min-h-screen bg-[#F8F7F4]">
      <div className="max-w-md mx-auto px-5 pt-6 pb-24">
        ...
        {/* Practical Tips — collapsed by default */}
          ...
          <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.25 }} className="mt-6">
            ...
            >
              <h3 className="text-[11px] font-bold text-gray-400 uppercase tracking-wider">
                Practical Tips ({guide.practicalTips.length})
              </h3>
              <svg
                width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#9CA3AF" strokeWidth="2"
                className={`transition-transform ${tipsExpanded ? 'rotate-180' : ''}`}
              >
                <polyline points="6 9 12 15 18 9" />
              </svg>
            </button>
            {tipsExpanded && (
              <div className="bg-white rounded-2xl border border-gray-100/80 p-4 space-y-2.5 mt-3">
        ...
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.45 }} className="mt-8">
      ...
      {/* Sticky Mark as Read bar at bottom */}
      {!isRead ? (
        <div className="sticky bottom-0 bg-white/95 backdrop-blur-sm border-t border-gray-100 px-5 py-3 z-20">
          <div className="max-w-md mx-auto">
            <button
              onClick={() => markGuideRead(guide.id)}
              className="w-full py-3 bg-[#4A7C59] text-white text-[15px] font-semibold rounded-2xl hover:bg-[#3d6849] transition-colors shadow-sm"
            >
              Mark as Read
            </button>
          </div>
        ...
        <div className="sticky bottom-0 bg-white/95 backdrop-blur-sm border-t border-gray-100 px-5 py-3 z-20">
          <div className="max-w-md mx-auto flex items-center justify-center gap-2 text-[#4A7C59]">
